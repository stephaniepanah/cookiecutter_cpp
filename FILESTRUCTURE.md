...
* C++ source files:
  * `include/cookiecutter_cpp/cookiecutter_cpp.hpp` is the main
    C++ header that declares the interface of your library.
  * `src/cookiecutter_cpp.cpp` is the main file that implements this library.
  * `app/cookiecutter_cpp_app.cpp` is an executable that uses the library.
    This can e.g. be used to provide a command line interface for your project.
  * `tests/cookiecutter_cpp_t.cpp` contains the unit tests for the library.
